...
import { Heart, Share, MessageCircle } from "lucide-vue-next";
...
const handleUpdateBlog = async (id: string) => {
  return navigateTo(`/admin/blog/update/${id}`);
};
    ...
    <Table>
      ...
      <TableBody>
        <TableRow v-for="(blog, index) in blogs" :key="blog._id">
          ...
          <TableCell
            ><div class="ql-editor line-clamp-3" v-html="blog.content"></div
          ></TableCell>
          <TableCell>
            <div class="grid grid-cols-3 gap-2 w-full">
              ...
              <div class="flex">
                ...
                <span class="text-sm">{{
                  Array.isArray(blog.sharedBy) ? blog.sharedBy.length : 0
                }}</span>
              </div>
          ...
          <TableCell>
            <Button @click="handleDeleteBlog(blog._id)"> Xóa </Button>
            <Button @click="handleUpdateBlog(blog._id)" class="ml-2">
              Sửa
            </Button>
          </TableCell>
        </TableRow>
